...
export default function SettingsModal({ fanAuth, onClose }: Props) {
  ...
  // ── Load this fan's support requests ────────────────────────────────────────
  useEffect(() => {
    if (!fanAuth) return
    const unsub = onSnapshot(
      query(
        collection(db, 'support'),
        where('fanId', '==', fanAuth.fanId),
  ...
  const handleCreateRequest = async () => {
    if (!fanAuth || !newTitle.trim() || !newSubject.trim() || creating) return
    setCreating(true)
    try {
      const now   = Date.now()
  ...
  const handleSendMsg = async () => {
    if (!fanAuth || !activeReq || !newMsg.trim() || sendingMsg) return
    setSendingMsg(true)
    const text = newMsg.trim(); setNewMsg('')
    try {
      const now = Date.now()
      await addDoc(collection(db, 'support', activeReq.id, 'messages'), {
        text, sender: 'user', senderName: fanAuth.fanName, timestamp: now,
      })
  ...
  const handleChangePin = useCallback(async () => {
    ...
    try {
      const snap = await getDoc(doc(db, 'fans', fanAuth.fanId))
      if (!snap.exists()) { setPinError('Account not found.'); return }
      const fan = snap.data()
      if (fan.pin !== oldPin)    { setPinError('Current PIN is incorrect.'); return }
      ...
      await updateDoc(doc(db, 'fans', fanAuth.fanId), { pin: newPin })
      setPinSuccess(true)
      setOldPin(''); setNewPin(''); setConfirmPin('')
      // Close modal and go home after success banner shows briefly
      setTimeout(() => { onClose(); router.replace('/rounds') }, 2000)
          ...
          {!fanAuth && (
            <>
              <div className="bg-club-red/5 border border-club-red/10 rounded-2xl p-4">
                <p className="font-bold text-gray-900 mb-1">KDFC Fan Zone</p>
                <p className="text-sm text-gray-500 leading-relaxed">
                  The official fan app for Keysborough District FC — track live scores,
                  vote for Man of the Match, and post messages to the team noticeboard.
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        {hasReply && (
                          <span className="shrink-0 w-2 h-2 rounded-full bg-club-red" />
                        )}
                      </div>
                ...
                return (
                  <div key={m.id} className={`flex gap-2 items-end ${isUser ? 'flex-row-reverse' : ''}`}>
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black shrink-0 ${
                      ...
                      {isUser ? fanAuth.fanName[0].toUpperCase() : 'A'}
                    </div>
                    <div>
